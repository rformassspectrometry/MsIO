...
#' @aliases loadMsObject,MsBackend,ANY-method
#'
#' @description
#'
#' The `saveMsObject()` and `loadMsObject()` methods allow serializing and
...
#'
#' - `loadMsObject(object, param)`: `object` defines the type of MS object that
...
#'     `loadMsObject()`: the MS data object that should be returned
#'
...
#'
#' @return `saveMsObject()` has no return value, `loadMsObject` is expected
...
#' @rdname saveMsObject
setGeneric("loadMsObject", function(object, param, ...)
    standardGeneric("loadMsObject"))
