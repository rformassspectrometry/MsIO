...
**MsIO** provides flexible, language-agnostic import and export
...
formats, making cross-platform data exchange difficult. **MsIO**
...
(e.g. `XcmsExperiment.R`, `Spectra.R`), with new formats expected to
...
- Tab-delimited export/import for key objects:
  - `MsBackendMzR`, `MsBackendCached`, `Spectra` (from
    [Spectra](https://github.com/RforMassSpectrometry/Spectra))
  - `MsBackendOfflineSql` (from
...
- Supported for `XcmsExperiment`, `MsExperiment`, `Spectra`,
  `MsBackendMzR`, `MsBackendCached`, `MsBackendOfflineSql`.

### ✅ mzTab-M Export (`mzTabParam`)

- Export of `XcmsExperiment` preprocessing results to **mzTab-M** (HUPO
  PSI metabolomics standard)

### ✅ MetaboLights (`MetaboLightsParam`)

- Import of complete experiments (including raw MS files) from
  [MetaboLights](https://www.ebi.ac.uk/metabolights/) as `MsExperiment`
  objects.
